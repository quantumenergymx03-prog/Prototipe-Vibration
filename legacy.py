            ...
            if xf is not None and mag_vel_mm is not None:
                ...
                try:
                    ...
                    if len(yv) > 0:
                        ...
                        # Calcular orden si RPM o dom_freq disponible
                        f1 = self._get_1x_hz(features_full.get("dom_freq", 0.0))
                        for pf, pa in zip(peak_f, peak_a):
                            order = (pf / f1) if (f1 and f1 > 0) else None
                            top_peaks.append((pf, pa, order))
                            label = f"{pf:.2f} Hz | {pa:.3f} mm/s"
                            # omit order text if not available
                            if order is not None:
                                try:
                                    label += f" | {order:.2f}X"
                                except Exception:
                                    pass
                            ax2.annotate(label, xy=(pf, pa), xytext=(5, 10), textcoords='offset points', fontsize=7, color="#e74c3c")
                except Exception:
                    pass
                ...
                try:
                    ...
                    for f0, label, col in marks:
                        if f0 and f0 > 0:
                            try:
                                ax2.axvline(f0, color=col, linestyle='--', alpha=0.8, linewidth=1.2)
                                ax2.text(f0, ymax*0.92, label, rotation=90, color=col, fontsize=7, va='top', ha='right')
                            except Exception:
                                pass
                except Exception:
                    pass
        ...
        try:
            ...
            if use_dbv:
                try:
                    ...
                    acc_plot = acc_spec[mask_vis] if ('mask_vis' in locals()) else acc_spec
            ...
            try:
                ...
                if len(yv) > 0:
                    ...
                    # Calcular orden si RPM está disponible o usar dom_freq
                    f1 = self._get_1x_hz(dom_freq)
                    for pf, pa in zip(peak_f, peak_a):
                        label = f"{float(pf):.2f} Hz | {float(pa):.3f} mm/s"
                        # omit order unless f1 is valid
                        if f1 and f1 > 0:
                            try:
                                label += f" | {pf/f1:.2f}X"
                            except Exception:
                                pass
                        ax2.annotate(label, xy=(pf, pa), xytext=(5, 10), textcoords='offset points',
            ...
            try:
                ...
                for f0, label, col in marks:
                    if f0 and f0 > 0:
                        try:
                            ax2.axvline(f0, color=col, linestyle='--', alpha=0.85, linewidth=1.2)
                            ax2.text(f0, ymax*0.92, label, rotation=90, color=col, fontsize=8, va='top', ha='right')
                        except Exception:
                            pass
            except Exception:
                pass
            ...
            try:
                ...
                if xf_env is not None and env_amp is not None and len(xf_env) > 0:
                    ...
                    try:
                        ...
                        if vis_peaks:
                            pfx, pfy = zip(*vis_peaks)
                            env_ax.scatter(pfx, pfy, color="#c0392b", s=24, zorder=5)
                            for f0, a0 in vis_peaks:
                                try:
                                    env_ax.annotate(f"{f0:.2f} Hz", xy=(f0, a0), xytext=(4, 8), textcoords='offset points', fontsize=8, color="#c0392b")
                                except Exception:
                                    pass
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        for f0, label, col in marks:
                            if f0 and f0 > 0:
                                try:
                                    env_ax.axvline(f0, color=col, linestyle='--', alpha=0.85, linewidth=1.2)
                                    env_ax.text(f0, ymax*0.92, label, rotation=90, color=col, fontsize=8, va='top', ha='right')
                                except Exception:
                                    pass
                    except Exception:
                        pass
